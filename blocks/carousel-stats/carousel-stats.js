...
function updateActiveSlide(slide) {
  ...
  // Disable prev on first slide, next on last slide (matches source behaviour)
  const prevBtn = block.querySelector('.slide-prev');
  const nextBtn = block.querySelector('.slide-next');
  if (prevBtn) prevBtn.toggleAttribute('disabled', slideIndex === 0);
  if (nextBtn) nextBtn.toggleAttribute('disabled', slideIndex === slides.length - 1);
}
